...
from ..inputs import config, schema
...
def dump_types() -> str:
    ...
    lines = [
        ...
        "# Regenerate with `./check.sh fix` "
        "(or `python -m abcmb._generators.schema_types`).",
...
def main() -> None:
    ...
    # __file__ is abcmb/_generators/schema_types.py, so parent.parent is the
    # abcmb package and its parent is the repo root.
    pkg = Path(__file__).resolve().parent.parent
    (pkg.parent / "defaults.toml").write_text(config.dump_defaults())
    (pkg / "inputs" / "_schema_types.py").write_text(dump_types())
