...
"""
c = jnp.float64(29979245800.0)  # Speed of light in cm s^{-1}
...
G = jnp.float64(
    1.18980205e-40
)  # Newton's gravitational constant (G/c^2), in cm^3 eV^{-1} s^{-2}
kB = jnp.float64(8.617343e-5)  # Boltzmann constant in eV / K

"""
Cosmology / particle physics related
"""
mp = jnp.float64(938271999.0)  # Proton rest mass, in eV
mn = jnp.float64(939565413.0)  # Neutron rest mass, in eV
...
E21 = jnp.float64(
    10.198714553953742
)  # Energy difference in n=1, 2 for hydrogen, in eV.
E31 = jnp.float64(
    12.087365397278509
)  # Energy difference in n=1, 3 for hydrogen, in eV.
E41 = jnp.float64(
    12.748393192442178
)  # Energy difference in n=1, 4 for hydrogen, in eV.
E32 = jnp.float64(
    1.8886508433247664
)  # Energy difference in n=2, 3 for hydrogen, in eV.
E42 = jnp.float64(
    2.5496786384884356
)  # Energy difference in n=2, 4 for hydrogen, in eV.
rydberg = jnp.float64(13.598286071938324)  # Ionization energy of hydrogen, in eV
lya_eng = rydberg * 3.0 / 4.0  # Lyman-alpha transition energy, in eV
lya_freq = lya_eng / (
    2.0 * jnp.pi * hbar
)  # Lyman-alpha transition frequency, in s^{-1}
thomson_xsec = jnp.float64(6.652458734e-25)  # Thomson cross section, in cm^2
stef_bolt = jnp.pi**2 / (
    60.0 * hbar**3 * c**2
)  # Stefan-Boltzmann constant, eV^{-3} cm^{-2} s^{-1}
