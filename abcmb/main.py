...
class Model(eqx.Module):
    ...
    def __init__(self, user_species=None, **kwargs):
        ...
        **kwargs : dict
        ...
        k_axis_perturbations, k_axis_Pk_output = model_specs.get_k_axis_perturbations(
            specs
        )
        ...
        if self.specs["bbn_type"].lower() == "linx":
            ...
            self.abundanceModel = AbundanceModel(
                NuclearRates(nuclear_net=self.specs["linx_reaction_net"]),
                adjoint=adjoint,
            )
        else:
    ...
    def add_derived_parameters(self, param_in: dict) -> dict:
        ...
        # Massive neutrinos
        params["T_nu_massive"] = jnp.array(
            params.get("T_nu_massive", 0.71611)
        )  # Massive neutrino temperature, as a ratio to TCMB.
        params["N_nu_massive"] = jnp.array(
            params.get("N_nu_massive", 0)
        )  # Number of massive neutrinos
        params["m_nu_massive"] = jnp.array(
            params.get("m_nu_massive", 0.06)
        ...
        if input_N and input_Neff:
            print(
                "You can only input one of N_nu_massless or Neff, but got values N_nu_massless={} and Neff={}.".format(
                    params["N_nu_massless"], params["Neff"]
                )
            )
            sys.exit()

        # If the user input either N_massless or Neff, but requested LINX, throw an error. LINX will compute the correct values.
        if (input_N or input_Neff or input_T_nu_massless) and self.specs[
            "bbn_type"
        ].lower() == "linx":
            print(
        ...
        params["T_nu_massless"] = jnp.array(
            params.get("T_nu_massless", 0.71636856)
        )  # Massless neutrino temperature, as a ratio to TCMB
        ...
        if self.specs["bbn_type"].lower() == "table":
            ...
            a_bbn = (
                cnst.TCMB_today * 1e-6 / 0.01
            )  # neutrino decoupling is well over by 10 keV, so
            ...
            res_YHe = bilinear_interp(
                omegab, DNeff, YHe_grid, params["omega_b"], Neff_BBN - 3.046
            )
            ...
            (
                ...
                P_NP_vec,
                Neff_vec,
            ) = eqx.filter_jit(self.thermo_model_DNeff, backend="cpu")(
                params["Delta_Neff_init"]
            )
            ...
            abundances = eqx.filter_jit(self.abundanceModel, backend="cpu")(
                ...
                nuclear_rates_q=jnp.asarray(
                    params.get(
                        "nuclear_rates_q",
                        jnp.zeros(len(self.abundanceModel.nuclear_net.reactions)),
                    )
                ),
            )
            ...
            try:
                ...
                params["Neff"] = jax.device_put(
                    Neff_vec[-1], device=jax.devices("gpu")[0]
                )
                YHe_BBN = jax.device_put(
                    4 * abundances[5], device=jax.devices("gpu")[0]
                )
            except:  # no GPU
                params["T_nu_massless"] = linxThermo.T_nu(
                    rho_nu_vec[-1]
                ) / linxThermo.T_g(rho_g_vec[-1])
                params["Neff"] = Neff_vec[-1]
                YHe_BBN = 4 * abundances[5]
                pass

            # CMB uses real mass fraction
            Yp_CMB = 1.0 / (4 * cnst.mH / cnst.mHe * (1 / YHe_BBN - 1) + 1)
            params["YHe"] = Yp_CMB
        ...
        params["omega_m"] = rho_m / (
            3 * cnst.H0_over_h**2 / 8 / jnp.pi / cnst.G
        ...
        )  # Fractional radiation density today
        params["R_nu"] = (
            rho_nu / rho_r
        )  # Fractional radiation density in neutrinos, defined at early times. Used for setting adiabatic ICs.
        ...
        params["omega_Lambda"] = (
            params["h"] ** 2 - params["omega_r"] - params["omega_m"]
        )
